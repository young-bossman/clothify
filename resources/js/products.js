document.addEventListener('DOMContentLoaded', () => {
    ...
    createImageWrapper?.addEventListener('dragover', e => { e.preventDefault(); createImageWrapper.classList.add('border-indigo-400'); });
    createImageWrapper?.addEventListener('dragleave', () => createImageWrapper.classList.remove('border-indigo-400'));
    createImageWrapper?.addEventListener('drop', e => {
        e.preventDefault();
        createImageWrapper.classList.remove('border-indigo-400');
        const file = e.dataTransfer.files[0];
        if (file) { createImageInput.files = e.dataTransfer.files; previewImage(file, createImagePreview, createImageText); }
    });
    ...
    editImageWrapper?.addEventListener('dragover', e => { e.preventDefault(); editImageWrapper.classList.add('border-indigo-400'); });
    editImageWrapper?.addEventListener('dragleave', () => editImageWrapper.classList.remove('border-indigo-400'));
    editImageWrapper?.addEventListener('drop', e => {
        e.preventDefault();
        editImageWrapper.classList.remove('border-indigo-400');
        const file = e.dataTransfer.files[0];
        if (file) { editImageInput.files = e.dataTransfer.files; previewImage(file, editImagePreview, editImageText); }
    });
    ...
    const loadProducts = async () => {
        ...
        try {
            const res = await fetch(`${baseUrl}/api/v1/products?${buildQuery()}`, { headers });
            if (!res.ok) throw new Error();
    ...
    const bindActions = () => {
        ...
        document.querySelectorAll('.editBtn').forEach(btn => {
            btn.onclick = async () => {
                try {
                    const res = await fetch(`${baseUrl}/api/v1/products/${btn.dataset.id}`, { headers });
                    if (!res.ok) throw new Error();
                    ...
                    if (editCategorySelect) editCategorySelect.value = p.category_id ?? '';
    ...
    if (productForm) {
        ...
        productForm.addEventListener('submit', async e => {
            ...
            formData.set('is_active', productForm.querySelector('[name="is_active"]').checked ? 1 : 0);
